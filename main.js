...
function myBrowser(){
    var userAgent = navigator.userAgent; //get broser userAgent string
...
var checkUsername=function(){
	...
	if(username==""||username==null){
		usernamecheck.innerText="name:cannot be null";
	}
	else{
		...
		usernamecheck.innerText="";		
			...
			usernamecheck.innerText="name:form wrong";
		}
	}
}
var checkMail=function(){
	...
	if(usermail==""||usermail==null){
		usermailcheck.innerText="mail:cannot be null";
	}
	else{
		...
		usermailcheck.innerText="";		
			...
			usermailcheck.innerText="email:form wrong";
		}
	}
}
var checkTheme=function(){
	...
	if(usertheme==""||usertheme==null){
		userthemecheck.innerText="theme:cannot be null";
	}
}
...
var checkCon=function(){
	...
	if(usercon==""||usercon==null){
		userconcheck.innerText="cannot be null";
	}
}
...
var scrollFunc = function (e) {
    ...
    var direct = 0;
    e = e || window.event;
    if (e.wheelDelta) {  //tell IE/Chrome : mouse scroll滑轮事件   
          
        if (e.wheelDelta > 0) { //when scroll up向上滚动
        ...
        if (e.wheelDelta < 0) { //when scroll down向下滚动
    ...
    } else if (e.detail) {  //Firefox : mouse scroll滑轮事件
        if (e.detail> 0) { //when scroll up向上滚动
            for (var i = 1; i < tag.length; i++) {
                if(tag[i].className=="cur_a"){
                    tag[i].className="";
                    con[i].style.display="none";
        ...
        if (e.detail< 0) { //when scroll down向下滚动
            for (var i = 0; i < tag.length-1; i++) {
                var n=0;
                if(tag[i].className=="cur_a"){
                    tag[i].className="";
